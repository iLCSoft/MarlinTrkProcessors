...
ExtrToSIT::ExtrToSIT() : Processor("ExtrToSIT") {
  ...
  _description = "ExtrToSIT refits an input track collection (TPC or VXD), and used IMarlinTrk tools to propagate it to SIT" ;
	  ...
	  if ( fit_status == 0 ){
		...
		if ( marlin_trk->propagateToLayer( layerID, trkState, chi2, ndf, elementID, IMarlinTrack::modeClosest) == MarlinTrk::IMarlinTrack::success) {
		  ...
		  if ( elementID != 0 ){
		    ...
		    */
		    //if (!_pixelSIT){
		      // Alternative way to assign the sensor s.p. resolution until I understand what's going on with that
		      dU_spres = 0.007;
		      dV_spres = 0.05;
		      //}
		      // ---------------------------------------------------------------------------------------------
			...
			//isSuccessful2 = marlin_trk->addAndFit( BestHit, chi2_increment, _Max_Chi2_Incr ) == IMarlinTrack::success;
		...
		all_hits.push_back(outliers[ihit].first);
	      ...
	      trackCandidates.push_back(refittedTrack) ;  // trackCandidates vector stores all the candidate tracks of the event
...
void ExtrToSIT::end(){ 
  ...
  streamlog_out(DEBUG) << "ExtrToSIT::end()  " << name() 
...
LCRelationNavigator* ExtrToSIT::GetRelations(LCEvent * evt , std::string RelName ) {
  ...
  try{
    ...
    streamlog_out( DEBUG2 ) << "ExtrToSIT --> " << RelName << " track relation collection in event = " << nav << std::endl;
  }
  catch(DataNotAvailableException &e){
    streamlog_out( DEBUG2 ) << "ExtrToSIT --> " << RelName.c_str() << " track relation collection absent in event" << std::endl;     
  }
