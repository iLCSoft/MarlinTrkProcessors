...
{ 
  ...
  _TREE = _TRF->create("DDTPCDigi.root",
                       "root",
                       false,
                       true);
...
void DDTPCDigiProcessor::processRunHeader( LCRunHeader* ) 
{ 
  _nRun++ ;
} 
...
{ 
  ...
  streamlog_out( DEBUG ) << "seed set to " << marlin::Global::EVENTSEEDER->getSeed(this) << " for event number "<< evt->getEventNumber() << std::endl;
  ...
  if( STHcol != 0 ){
    ...
    for(int i=0; i< n_sim_hits; i++){
      ...
      if (driftLength <0) { 
        streamlog_out(DEBUG3) << " DDTPCDigiProcessor : Warning! driftLength < 0 " << driftLength << " --> wrong data in dd4hep::rec::FixedPadSizeTPCData ? " << std::endl; 
      ...
      int iZHit = (int) ( (float) NBinsZ * ( _tpc->driftLength/dd4hep::mm + thisPoint.z() ) / ( 2.0 * _tpc->driftLength/dd4hep::mm ) ) ;
  ...
  if(STHcolLowPt!=NULL){
    ...
    for(int i=0; i< n_sim_hitsLowPt; i++){  
      ...
      int iZHit = (int) ( (float) NBinsZ * 
                         ( _tpc->driftLength/dd4hep::mm + thisPoint.z() ) / ( 2.0 * _tpc->driftLength/dd4hep::mm ) ) ;
...
{ 
  ...
  streamlog_out(MESSAGE) << "DDTPCDigiProcessor::end()  " << name() 
...
double DDTPCDigiProcessor::getPadPhi( CLHEP::Hep3Vector *thisPoint, CLHEP::Hep3Vector* firstPoint, CLHEP::Hep3Vector* middlePoint, CLHEP::Hep3Vector* lastPoint){
  ...
  if( (fabs( firstPointRPhi.x() - middlePointRPhi.x() ) < 1.e-05  && fabs( firstPointRPhi.y() - middlePointRPhi.y() ) < 1.e-05) 
    ...
    streamlog_out(WARNING) << " DDTPCDigiProcessor::getPadPhi "  
...
double DDTPCDigiProcessor::getPadTheta(CLHEP::Hep3Vector* firstPoint, CLHEP::Hep3Vector* middlePoint, CLHEP::Hep3Vector* lastPoint){
  ...
  if( (fabs( firstPointRPhi.x() - middlePointRPhi.x() ) < 1.e-05  && fabs( firstPointRPhi.y() - middlePointRPhi.y() ) < 1.e-05) 
    ...
    streamlog_out(WARNING) << " DDTPCDigiProcessor::getPadTheta "  
    << "2 of the 3 SimTracker hits passed to Circle Fit are the same hit taking pad phi as PI/2\n"
    << " firstPoint->x() "  << firstPoint->x() 
    << " firstPoint->y() "  << firstPoint->y() 
    << " firstPoint->z() "  << firstPoint->z() 
