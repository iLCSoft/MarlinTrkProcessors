...
FixedPadSizeDiskLayout::~FixedPadSizeDiskLayout() { cleanup(); }
...
FixedPadSizeDiskLayout::FixedPadSizeDiskLayout(const dd4hep::rec::FixedPadSizeTPCData* tpc)
    : _rMin(tpc->rMinReadout / dd4hep::mm), _rMax(tpc->rMaxReadout / dd4hep::mm), _phiMax(2. * M_PI),
      _padWidth(tpc->padWidth / dd4hep::mm), _padHeight(tpc->padHeight / dd4hep::mm), _padGap(tpc->padGap / dd4hep::mm),
      _nRow(tpc->maxRow) {
  _extent.resize(4);
  _extent[0] = _rMin;
  _extent[1] = _rMax;
  ...
  int nr = ((_rMax + 0.01 - _rMin) / _padHeight);

  _nRow = (_nRow > 0) ? _nRow : nr;

  if (_nRow > nr) {
    std::cout << " WARNING: FixedPadSizeDiskLayout() :  cannot place  " << _nRow << " rows of height " << _padHeight
              << " mm between rMin = " << _rMin << " mm and rMax = " << _rMax << " mm -  will use nRow = " << nr
              << " !! " << std::endl;
  ...
  for (std::vector<std::vector<int>*>::iterator rowIter = _padIndices.begin(); rowIter < _padIndices.end(); rowIter++) {
    *rowIter = 0;
  }

  _rowHeight = (_rMax - _rMin) / _nRow;
  ...
  for (int i = 0; i < _nRow; i++) {
    ...
    row.RCenter = _rMin + (i * _rowHeight) + .5 * _rowHeight;

    double u = row.RCenter * _phiMax;

    row.NPad = static_cast<int>(std::floor(u / (_padWidth + _padGap)));
...
FixedPadSizeDiskLayout::FixedPadSizeDiskLayout(const FixedPadSizeDiskLayout& right) { copy_and_assign(right); }
...
int FixedPadSizeDiskLayout::getNRows() const { return _rows.size(); }
...
dd4hep::rec::Vector2D FixedPadSizeDiskLayout::getPadCenter(int padIndex) const {
  int rowNum = getRowNumber(padIndex);
  ...
  return dd4hep::rec::Vector2D(r, phi);
}
...
int FixedPadSizeDiskLayout::getPadIndex(int rowNum, int padNum) const {
  ...
  if (padNum > _rows[rowNum].NPad - 1) {
    ...
    sstr << "FixedPadSizeDiskLayout::getPadIndex: pad number too large: " << padNum << " only " << _rows[rowNum].NPad
         << " pads in row " << rowNum;
...
int FixedPadSizeDiskLayout::getNearestPad(double r, double phi) const {
  if (fabs(r) < 1e-6)
    throw std::runtime_error("FixedPadSizeDiskLayout::getNearestPad: r can't be zero !");
  ...
  // Expensive condition checkings are moved towards the end.
    ...
    std::cout << "I have no idea where the nearest pad is for the point (" << r << ", " << phi << ")" << std::endl;
...
int FixedPadSizeDiskLayout::getLeftNeighbour(int padIndex) const {
  ...
  if (pn > nPad - 1) {
    ...
    if (fabs(_phiMax - 2 * M_PI) < 1e-6)
      ...
      throw std::runtime_error("FixedPadSizeDiskLayout::getLeftNeighbour: no left neighbour pad !");
  }
...
int FixedPadSizeDiskLayout::getRightNeighbour(int padIndex) const {
  ...
  if (pn < 0) {
    ...
    if (fabs(_phiMax - 2 * M_PI) < 1e-6)
      ...
      throw std::runtime_error("FixedPadSizeDiskLayout::getRightNeighbour: no right neighbour pad !");
  }
...
bool FixedPadSizeDiskLayout::isInsidePad(double r, double phi, int padIndex) const {
  ...
  dd4hep::rec::Vector2D p = getPadCenter(padIndex);
  ...
  return (std::abs(r - p[0]) <= 0.5 * _padHeight && std::abs(phi - p[1]) <= phiPadHalf);
}
